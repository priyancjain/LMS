#!/usr/bin/env python3
"""
Create 10 comprehensive learning flows covering various CS topics.
"""
import sqlite3
import json
import uuid
...
def create_flows():
    ...
    for flow_data in FLOWS:
        ...
        cursor.execute("SELECT flow_id FROM flows WHERE title = ?", (flow_data["title"],))
        if cursor.fetchone():
            print(f"✓ Flow '{flow_data['title']}' already exists")
            continue
        ...
        cursor.execute("""
            ...
            VALUES (?, ?, ?, ?)
        """, (flow_id, flow_data["title"], TEACHER_EMAIL, datetime.now().isoformat()))
        ...
        for q_data in flow_data["questions"]:
            ...
            cursor.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """, (question_id, flow_id, content, options_json, correct, difficulty, hint, datetime.now().isoformat()))
        
        print(f"✓ Created '{flow_data['title']}' with {len(flow_data['questions'])} questions")
...
if __name__ == "__main__":
    try:
        create_flows()
        print("\n✓ All 10 flows created successfully!")
